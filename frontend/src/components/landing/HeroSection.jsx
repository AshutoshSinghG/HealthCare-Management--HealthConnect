...
import { Activity, ArrowRight, Play, ChevronDown, Smile } from 'lucide-react';
...
// ─────────────────────────────────────────────
// Background video (left / full-screen layer)
// ─────────────────────────────────────────────
const HeroVideoBackground = ({ clipIndex, handleEnded }) => {
  const videoRef = useRef(null);
  const [loaded, setLoaded] = useState(false);

  // FIX 1: jab bhi clipIndex badle, naya src load karke play karo
  useEffect(() => {
...
const HeroSection = () => {
  ...
  // FIX 2: Right-panel videos ke liye ref array
  ...
  useEffect(() => {
    rightVideoRefs.current.forEach((vid, i) => {
      if (!vid) return;
      if (i === clipIndex) {
        vid.currentTime = 0;
        vid.play().catch(() => { });
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center overflow-hidden bg-surface-900">
      ...
      <svg className="absolute bottom-32 left-0 w-full h-16 opacity-[0.10]" viewBox="0 0 1600 60" fill="none" style={{ zIndex: 6 }}>
        <path
          className="ecg-line"
          d="M0 30 L300 30 L340 30 L360 8 L380 52 L400 5 L420 55 L440 25 L460 35 L500 30
             L800 30 L840 30 L860 8 L880 52 L900 5 L920 55 L940 25 L960 35 L1000 30
             L1300 30 L1340 30 L1360 8 L1380 52 L1400 5 L1420 55 L1440 25 L1460 35 L1500 30 L1600 30"
          stroke="white"
          strokeWidth="2"
        />
      </svg>
          ...
          >
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-3">
                {['bg-blue-400', 'bg-green-400', 'bg-purple-400', 'bg-orange-400', 'bg-pink-400'].map(
                  (bg, i) => (
                    <div
                      key={i}
                      className={`w-10 h-10 rounded-full ${bg} border-2 border-white/20 flex items-center justify-center text-white text-xs font-bold`}
                    >
                      {['Dr', 'Pt', 'Dr', 'Ad', 'Pt'][i]}
                    </div>
                  )

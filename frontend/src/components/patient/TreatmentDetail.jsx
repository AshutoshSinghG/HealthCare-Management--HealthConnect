...
import { ArrowLeft, Calendar, User, Stethoscope, FileText, Pill, ClipboardList, Loader2 } from 'lucide-react';
...
const TreatmentDetail = () => {
  ...
  if (isError || !data) return (
    <div className="text-center py-20 text-surface-500">
      <p>Failed to load treatment details.</p>
      <Link to="/patient/treatments"><Button variant="ghost" className="mt-4">Go Back</Button></Link>
    </div>
  );

  const treatment = data.treatment;
  const medications = data.medications || [];
  const doctorName = treatment.doctorId
    ? `Dr. ${treatment.doctorId.firstName} ${treatment.doctorId.lastName}`
    : 'Unknown';
  const specialisation = treatment.doctorId?.specialisation || '';
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <Badge variant={outcomeColors[treatment.outcomeStatus] || 'info'} size="sm" dot>{treatment.outcomeStatus}</Badge>
              </div>
            </div>
          ...
          <Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-surface-500">{specialisation}</p>
              </div>
            </div>
          </Card>
